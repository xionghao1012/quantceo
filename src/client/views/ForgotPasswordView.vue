...
async function handleSubmit() {
  ...
  error.value = ''
  try {
    const res = await fetch('http://localhost:3002/api/auth/password-reset/send', {
      method: 'POST',
  ...
  <div class="auth-page">
    <div class="card auth-card">
      ...
      <div v-if="success" class="success-box">
        <p>✅ 密码重置链接已发送到您的邮箱</p>
        <p class="hint">请查收邮件并在 1 小时内点击链接重置密码。</p>
...
.success-box .hint { font-size: 12px; color: var(--text-secondary, #aaa); }
.success-box .btn-primary { margin-top: 8px; }
.btn-primary {
  width: 100%;
  padding: 10px;
